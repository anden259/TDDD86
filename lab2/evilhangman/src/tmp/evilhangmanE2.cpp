...
//template<typename T>
pair<string, vector<string> > get_biggest_word_list(map<string, vector<string> >& family_map) {
...
//template<typename T>
pair<string, vector<string> > get_most_unique_letters(map<string, vector<string> >& family_map) {
    pair <string, vector<string> > ret_map_pair;
    int max_len(0), tmp(0);
...
void play_game(int len, int gue, bool show, vector<string> word_list) {
    ...
    while(word_to_guess.find('-') != string::npos && gue > 0) {
        ...
        if (gue>10) {
            word_list_pair = get_biggest_word_list(family);
        } else {
            word_list_pair = get_most_unique_letters(family);
        }
